...
# $HOME/Downloads is deliberately not probed -- whatever is found gets its
# completion.fish.inc sourced by every shell, and a browser download target
# must not be trusted with that (TODO #43).
...
# Everything to the matching `end` is for interactive shells only. Without
# this guard every `fish -c script` paid for the prompt, the aliases and the
# abbreviations.
if status is-interactive
    ...
    #
    # if/else rather than switch: fish treats "?" in a case pattern as a
    # single-character wildcard even when quoted.
    ...
    function _git_prompt_info --description 'Coloured git branch + status fragment'
        ...
        set -l unstaged ""
        set -l untracked ""
        for line in (git -c core.fsmonitor= -c diff.external= status --porcelain=v2 --branch 2>/dev/null)
            set -l tag (string sub -l 2 -- $line)
            ...
            else if test "$tag" = "1 " -o "$tag" = "2 "
                set -l xy (string sub -s 3 -l 2 -- $line)
                test (string sub -l 1 -- $xy) != "."; and set staged "+"
                test (string sub -s 2 -l 1 -- $xy) != "."; and set unstaged "*"
            else if test "$tag" = "u "
                set unstaged "*"
            else if test "$tag" = "? "
                set untracked "?"
        ...
        echo -n " "(set_color brblue)"("(set_color brcyan)"$branch$staged$unstaged$untracked"(set_color brblue)")"(set_color normal)
    end
    ...
    function fish_prompt
        ...
        # Colours match bash and zsh: green user, yellow @, green host, cyan path.
        printf '%s%s%s%s%s%s%s: %s%s%s%s\n> %s' \
            (set_color brgreen) $USER \
            (set_color bryellow) '@' \
            (set_color brgreen) (prompt_hostname) (set_color normal) \
            (set_color brcyan) (prompt_pwd) (set_color normal) \
            "$vcs_info" \
            (set_color normal)
    end
...
switch (uname)
    ...
    case Darwin
        ...
        # Only load keychain keys if the agent has none yet; quiet, never
        # blocking, and only for a shell a human is actually using.
        if status is-interactive; and not ssh-add -l >/dev/null 2>&1
...
# Interactive-only: a prompt hook and shell completions do nothing in a script.
if status is-interactive
    ...
    # Kubectl completion. bash had this and the other shells did not, so
    # completions differed by shell.
...
# Note: NVM and RVM do not natively support Fish. They were removed from the
# bash config too -- none is installed, and direnv covers the same ground.
...
# Machine-specific settings that must not be committed. The bash/zsh
# equivalent is ~/.shell_local, which fish cannot source because it is POSIX.
